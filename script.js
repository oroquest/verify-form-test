...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const decodeB64Url = (s) => {
    ...
    try { return decodeURIComponent(escape(atob(t))); } catch (e) {
      try { return atob(t); } catch { return ""; }
    }
  };
  const email = decodeB64Url(em).trim().toLowerCase();
  if (email) {
  ...
  try {
    const response = await fetch(`/.netlify/functions/get_contact?email=${encodeURIComponent(email)}`);
