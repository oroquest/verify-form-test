...
let currentLang = "de"; // Standard

function setLanguage(lang) {
  currentLang = lang; // Globale Variable setzen
  const i18n = {
    de: {
      title: "Bitte bestätigen Sie Ihre Daten",
      glaeubiger: "Gläubiger-Nr.",
...
// Sprachwechsel beim Laden initialisieren
document.addEventListener("DOMContentLoaded", () => {
  const params = new URLSearchParams(window.location.search);
  const glaeubigerId = params.get("id") || "";
  const token = params.get("token") || "";
  ...
  // Setze Sprache initial auf Deutsch
...
document.getElementById("verify-form").addEventListener("submit", function(event) {
  ...
  const messages = {
    ...
    it: {
      email: "Si prega di inserire l'indirizzo e-mail.",
