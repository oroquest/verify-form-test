// Klassisch, robust gegen Mailjet-"glÃ¤ubiger", fetch JSON-only submit
const i18n = {
...
function validateFrontend(dict){
  const req=["firstname","name","strasse","hausnummer","plz","ort","country"]; let ok=true;
  ...
  const plzEl=document.getElementById("plz"); plzEl.value=plzEl.value.replace(/\D+/g,""); const plz=plzEl.value;
...
(async () => {
  ...
  const email=(b64urlDecode(em)||qs('email')||'').trim().toLowerCase();
  ...
  try{
    ...
    // gläubiger robust lesen (inkl. glÃ¤ubiger)
    let glaeubiger = (
      data.glaeubiger ??
      data['gläubiger'] ??
      data['glÃ¤ubiger'] ??
    ...
    glaeubiger = (glaeubiger ?? '').toString().trim();
    if(!glaeubiger) glaeubiger = id; // Fallback: E‑Mail enthielt oft keine Nummer → zeigen, aber prüfen tun wir nur serverseitig
    setVal('glaeubiger',glaeubiger);

    ['firstname','name','strasse','hausnummer','plz','ort','country'].forEach(k=> setVal(k, data[k]));

    okBox.textContent=(i18n[currentLang]||i18n.de).gateOk;
  ...
  // Submit (nur Email/Id/Token, JSON)
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      errBox.textContent = `${dict.errGeneric} (Server: ${resp.status}) — ${text}`;
      errBox.classList.remove('hidden');
    } catch {
